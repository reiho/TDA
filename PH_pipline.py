...
import numpy as np
import pandas as pd
from mapper_pipline import clusterization_results, clustering, distance
...
def PL(X, p, folder_name, sign):
    if len(p)>5000:
        most_var = list(((X.var()-X.min()) / (X.max()-X.min())).sort_values()[-5000:].index)
    ...
    XPL = []
    #print(min(len(p), 10))
    for tmax in range(min(len(p), 10), max(int(len(p)/10), int(X.shape[0]/10)), 10):
        #print(tmax)
        change=False
        XPL = []
